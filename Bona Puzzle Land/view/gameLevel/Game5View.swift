...
struct Game5View: View {
    ...
    @State private var imagePositions: [CGPoint] = Array(repeating: .zero, count: 6) // Positions for six images

    @State private var isFullScreenPresented = false
    @State private var restartViewID = UUID()

    @State private var imgs: [Bool] = Array(repeating: false, count: 6) // States for six images
    ...
    @State private var frames: [CGRect] = Array(repeating: .zero, count: 6) // Frames of all images
    @State private var positions: [CGSize] = Array(repeating: .zero, count: 6) // Positions of parts
    @State private var placedParts: [Bool] = Array(repeating: false, count: 6) // Tracking placed parts
    ...
    let puzzleParts = ["u1", "u2", "u3", "u4", "u5", "u6"] // Names of six parts
    ...
    var body: some View {
        GeometryReader { geometry in
            ZStack {
                ...
                VStack {
                    ...
                    VStack {
                        ...
                        HStack {
                            ...
                            VStack{
                                puzzlePieceView(imageName: !imgs[4] ? "Rectangle 64" : puzzleParts[4], index: 4, width: 120, height: 60)
                                puzzlePieceView(imageName: !imgs[5] ? "Rectangle 64" : puzzleParts[5], index: 5, width: 120, height: 60)
                            }
                        }
                    }.padding(.top, 150)
                        .padding(.bottom, 100)
                ...
                if count == 6 { // Updated condition for six puzzles
                    VStack {
                        ZStack {
                            Image("win")
                            Button {
