...
struct Game6View: View {
    ...
    @State private var imagePositions: [CGPoint] = Array(repeating: .zero, count: 7) // Positions for seven images

    @State private var isFullScreenPresented = false
    @State private var restartViewID = UUID()

    @State private var imgs: [Bool] = Array(repeating: false, count: 7) // States for seven images
    ...
    @State private var frames: [CGRect] = Array(repeating: .zero, count: 7) // Frames of all images
    @State private var positions: [CGSize] = Array(repeating: .zero, count: 7) // Positions of parts
    @State private var placedParts: [Bool] = Array(repeating: false, count: 7) // Tracking placed parts
    ...
    let puzzleParts = ["o1", "o2", "o3", "o4", "o5", "o6", "o7"] // Names of seven parts
    ...
    var body: some View {
        GeometryReader { geometry in
            ZStack {
                ...
                if count == 7 { // Updated condition for seven puzzles
                    VStack {
                        ZStack {
                            Image("win")
                            Button {
...
struct Game6View_Previews: PreviewProvider {
