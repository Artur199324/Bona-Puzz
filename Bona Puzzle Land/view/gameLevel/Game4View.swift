...
struct Game4View: View {
    ...
    @State private var imagePositions: [CGPoint] = Array(repeating: .zero, count: 5) // Positions for five images

    @State private var isFullScreenPresented = false
    @State private var restartViewID = UUID()

    @State private var imgs: [Bool] = Array(repeating: false, count: 5) // States for five images
    ...
    @State private var frames: [CGRect] = Array(repeating: .zero, count: 5) // Frames of all images
    @State private var positions: [CGSize] = Array(repeating: .zero, count: 5) // Positions of parts
    @State private var placedParts: [Bool] = Array(repeating: false, count: 5) // Tracking placed parts
    ...
    let puzzleParts = ["с1", "с2", "с3", "с4", "с5"] // Names of five parts
    ...
    var body: some View {
        GeometryReader { geometry in
            ZStack {
                    ...
                    VStack {
                        HStack {
                            VStack{
                                ...
                                puzzlePieceView(imageName: !imgs[2] ? "Rectangle 64" : puzzleParts[2], index: 2, width: 120, height: 60)
                            }
                        ...
                        HStack {
                            ...
                            puzzlePieceView(imageName: !imgs[4] ? "Rectangle 64" : puzzleParts[4], index: 4, width: 120, height: 120)
                        }
                ...
                if count == 5 { // Updated condition for five puzzles
                    VStack {
                        ZStack {
                            Image("win")
                            Button {
