...
struct Game3View: View {
    ...
    @State private var imagePositions: [CGPoint] = Array(repeating: .zero, count: 4) // Positions for four images

    @State private var isFullScreenPresented = false
    @State private var restartViewID = UUID()

    @State private var imgs: [Bool] = Array(repeating: false, count: 4) // States for four images
    ...
    @State private var frames: [CGRect] = Array(repeating: .zero, count: 4) // Frames of all images
    @State private var positions: [CGSize] = Array(repeating: .zero, count: 4) // Positions of parts
    @State private var placedParts: [Bool] = Array(repeating: false, count: 4) // Tracking placed parts
    ...
    let puzzleParts = ["t1", "t2", "t3", "t4"] // Names of four parts
            ...
            ZStack {
                ...
                VStack {
                    ...
                    VStack {
                        ...
                        HStack {
                            puzzlePieceView(imageName: !imgs[0] ? "Rectangle 64" : puzzleParts[0], index: 0, width: 120, height: 120)
                            puzzlePieceView(imageName: !imgs[1] ? "Rectangle 64" : puzzleParts[1], index: 1, width: 120, height: 120)
                            
                        }
                        HStack{
                            puzzlePieceView(imageName: !imgs[2] ? "Rectangle 64" : puzzleParts[2], index: 2, width: 120, height: 120)
                            puzzlePieceView(imageName: !imgs[3] ? "Rectangle 64" : puzzleParts[3], index: 3, width: 120, height: 120)
                        }
                ...
                if count == 4 { // Updated condition for four puzzles
                    VStack {
                        ZStack {
                            Image("win")
                            Button {
                ...
                .gesture(
                    DragGesture()
                        ...
                        .onEnded { value in
                            ...
                            print("Coordinates of current object (on release): \(self.currentFrame)")
                            ...
                            for (index, position) in imagePositions.enumerated() {
                                ...
                                if self.checkIntersectionWithTolerance(self.currentFrame, targetFrame, tolerance: tolerance) {
                                    if currentPartIndex == index {
                                        switch index {
                                        case 0: imgs[0].toggle()
                                        case 1: imgs[1].toggle()
                                        case 2: imgs[2].toggle()
                                        case 3: imgs[3].toggle()
                                        default: break
                                        }
                                        count += 1
    ...
    func increaseAndSaveValue(by amount: Int) {
           ...
           print("Updated value: \(savedValue)")
       }
}
